...
causalgraph.model.load_weights(save_path)

batch = 0
ds_test = cfg.get_ds_test()
print('+' * 30)
print(ds_test.num_examples)

print('Start evaluation...')
print('==============================================================')
# causalgraph.model.save_weights(cfg.save_path)
...
for i in range(ds_test.batches):
    ...
    for j in range(len(joint_label)):
        ...
        if sum(lb_idx) == 5:
            pScore = forgery_joint_score[j, lb_idx]
            nScore = forgery_joint_score[j, ~lb_idx]
            rec = recallAtK(pScore, nScore, top_25)
            prc = precisionAtK(pScore, nScore, top_5)
            # collect results
        ...
        mrr = 1 / rank
        pScore = frame_score_j[frame_label[j]]
